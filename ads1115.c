...
//   const bool ret = true; // dummy value to pass to queue
//   QueueHandle_t gpio_evt_queue = (QueueHandle_t) arg; // find which queue to write
//   xQueueSendFromISR(gpio_evt_queue, &ret, NULL);
// }


// ISR handler for the GPIO interrupt
static void IRAM_ATTR gpio_isr_handler(void* arg) {
    const bool ret = true; // dummy value to pass to queue
    xQueueSendFromISR(gpio_evt_queue, &ret, NULL); // Send the value to the queue
}

void setup_gpio_interrupt(gpio_num_t gpio_pin) {
 
    // Install GPIO ISR service
    if (gpio_install_isr_service(ESP_INTR_FLAG_LEVEL1) != ESP_OK) {
        ESP_LOGE("GPIO", "GPIO ISR service installation failed");
        return;
    }
    ...
    gpio_config_t io_conf = {
        ...
        .intr_type = GPIO_INTR_NEGEDGE, // or GPIO_INTR_POSEDGE or GPIO_INTR_ANYEDGE
        .pull_up_en = GPIO_PULLUP_ENABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE
    };
    gpio_config(&io_conf);

    // Create a queue for the GPIO interrupt event
    gpio_evt_queue = xQueueCreate(10, sizeof(bool));

    // Add the GPIO ISR handler
    if (gpio_isr_handler_add(gpio_pin, gpio_isr_handler, NULL) != ESP_OK) {
        ESP_LOGE("GPIO", "Failed to add ISR handler for GPIO%d", gpio_pin);
        return;
    }
}
...
static esp_err_t ads1115_write_register(ads1115_t* ads, ads1115_register_addresses_t reg, uint16_t data) {
  ...
  out[0] = (data >> 8)& 0xFF; // get 8 greater bits
...
double ads1115_get_voltage(ads1115_t* ads) {
  ...
  return (double)raw*5.95*fsr[ads->config.bit.PGA] / (double)bits;
}
double ads1115_get_current(ads1115_t* ads) {
  const double fsr[] = {6.144, 4.096, 2.048, 1.024, 0.512, 0.256};
  const int16_t bits = (1L<<15)-1;
  int16_t raw;
  raw = ads1115_get_raw(ads);
